...
| 演算子 | 記号 | 説明 | 対応するSQL |
| :--- | :---: | :--- | :--- |
| **選択 (Select)** | $\sigma$ | 選択述語（条件）を満たすタプルのサブセットを出力する。 | `WHERE` 句 |
| **射影 (Projection)** | $\pi$ | 指定された属性のみを含むリレーションを出力する。 | `SELECT` リスト |
| **和 (Union)** | $\cup$ | 両方の入力リレーションの少なくとも一方に存在するタプルを出力。 | `UNION` |
| **交わり (Intersection)** | $\cap$ | 両方の入力リレーションに存在するタプルを出力。 | `INTERSECT` |
| **差 (Difference)** | $-$ | 最初の入力には存在し、2番目の入力には存在しないタプルを出力。 | `EXCEPT` |
| **直積 (Product)** | $\times$ | 2つのリレーションのタプルのすべての可能な組み合わせを出力する。 | `CROSS JOIN` |
| **結合 (Join)** | $\bowtie$ | 共有する属性の値が一致するタプルの組み合わせを出力する。 | `INNER JOIN` など |
